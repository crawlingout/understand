...
0,ui_loc={en:{day:"day",days:"days"},cs:{day:"den",days:"dn\u016f"}},tracking_interval=0,today=moment().format("YYYY-MM-DD"),allowed_idle=3E5,last_pause_time,pause_diff,data=JSON.parse(localStorage.getItem("data"))||{};function convertSeconds(a){var b="0 min",c;if(0<a){a=moment.duration(a,"seconds");(b=a.hours()||"")&&(b+=" h ");if((c=a.minutes()||"")||!b)c=a.minutes()+" min";b+=c}return b}function calculateRatio(a,b){return a&&b?Math.floor(a/b*100):"-"}
...
TRACK.addAudioTime=function(a){-7<a&&1>a&&(0<a?(TRACK.addToDayAndTotal(a,"st"),TRACK.addToDayAndTotal(a,"at")):0<data[from].days[today].at+a&&TRACK.addToDayAndTotal(a,"at"))};TRACK.startPauseTimer=function(){last_pause_time=moment()};TRACK.addPauseTime=function(){last_pause_time&&(pause_diff=moment().diff(last_pause_time),pause_diff<allowed_idle&&TRACK.addToDayAndTotal(pause_diff/1E3,"st"))};
...
linkToDict(a))}else $("#googletranslate").attr("href","https://translate.google.com/#"+from+"/"+to+"/"+encodeURIComponent(a)),$("#whentoolong").removeClass("hidden"),$("#translations").addClass("hidden");""!==b[from]&&"..."!==b[to]&&$("#previous_translated_words_wrapper").is(":visible")&&prependPrevWord(b)}}function setKnob(a,b){var c=b||stored_audio_time;$("#knob_player").trigger("configure",{max:a}).val(c).trigger("change")}
...
loadDemo($(this).attr("id"));$("#from").val($(this).parent().data("lang"));$(".select_lang").change()});$("#tryitnow").click(function(){$("#more").removeClass("hidden");$("html, body").scrollTop(0)});$(".overlay").click(function(){$(".hidepopup").addClass("hidden");$("html, body").scrollTop(0)});$(".closepopup").click(function(){$(".hidepopup").addClass("hidden");$("html, body").scrollTop(0)});window.onkeyup=function(b){b=b.keyCode?b.keyCode:b.which;37==b?jumpBack(a):39==b&&playPause()};$("#qr").hover(function(){$("#qrimage").removeClass("hidden")},
function(){$("#qrimage").addClass("hidden")});var d=$("#r").text(),d=d.replace(" at ",String.fromCharCode(64)),d=d.replace(" dot ",String.fromCharCode(46));$("#r").text(d);$(".go_to_faq").click(function(){$("html, body").animate({scrollTop:$("#faq").offset().top},1E3)});TRACK.dataObjectExist(today);TRACK.setTodayKnob();TRACK.displayTrackingData(today);data[from].days[today].ga&&($("#i_am_done").removeClass("hidden"),$("#idle").html('<i class="fa fa-check-circle"></i>'));TRACK.currentStreak();TRACK.ratioStats();
$("#idle").click(function(){$("html, body").animate({scrollTop:$("#tracking")[0].scrollHeight+($("#tracking").offset().top-$(window).height())})});$("#minus").click(function(){900<data[from].total.dg&&(data[from].total.dg-=900,TRACK.setTodayKnob(),TRACK.storeTrackingData())});$("#plus").click(function(){data[from].total.dg+=900;TRACK.setTodayKnob();TRACK.storeTrackingData()})});