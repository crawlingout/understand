...
setInterval(function(){player.paused||player.ended?(last_pause_time&&moment().diff(last_pause_time)>allowed_idle&&(document.getElementById("idle").style.color="#d7d7d7"),TRACK.newDay()):TRACK.displayTrackingData(today)},1E3);$("#idle").click(function(){$("html, body").animate({scrollTop:$("#tracking")[0].scrollHeight+($("#tracking").offset().top-$(window).height())})});$("#minus").click(function(){900<data[from].total.dg&&(data[from].total.dg-=900,TRACK.setTodayKnob(),TRACK.storeTrackingData())});
$("#plus").click(function(){data[from].total.dg+=900;TRACK.setTodayKnob();TRACK.storeTrackingData()})});