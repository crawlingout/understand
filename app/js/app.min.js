...
loadDemo($(this).attr("id"));$("#from").val($(this).parent().data("lang"));$(".select_lang").change()});$("#tryitnow").click(function(){$("#more").removeClass("hidden");$("html, body").scrollTop(0)});$(".overlay").click(function(){$(".hidepopup").addClass("hidden");$("html, body").scrollTop(0)});$(".closepopup").click(function(){$(".hidepopup").addClass("hidden");$("html, body").scrollTop(0)});window.onkeyup=function(b){b=b.keyCode?b.keyCode:b.which;37==b?jumpBack(a):39==b&&playPause()};var d=$("#r").text(),
d=d.replace(" at ",String.fromCharCode(64)),d=d.replace(" dot ",String.fromCharCode(46));$("#r").text(d);$(".jump_to").click(function(){var a=$(this).data("marker");$("html, body").animate({scrollTop:$("#"+a).offset().top},1E3)});TRACK.dataObjectExist(today);TRACK.setTodayKnob();TRACK.displayTrackingData(today);data[from].days[today].ga&&($("#i_am_done").removeClass("hidden"),$("#idle").html('<i class="fa fa-check-circle"></i>'));TRACK.currentStreak();TRACK.ratioStats();setInterval(function(){player.paused||
player.ended?(last_pause_time&&moment().diff(last_pause_time)>allowed_idle&&(document.getElementById("idle").style.color="#d7d7d7"),TRACK.newDay()):TRACK.displayTrackingData(today)},1E3);$("#idle").click(function(){$("html, body").animate({scrollTop:$("#tracking")[0].scrollHeight+($("#tracking").offset().top-$(window).height())})});$("#minus").click(function(){900<data[from].total.dg&&(data[from].total.dg-=900,TRACK.setTodayKnob(),TRACK.storeTrackingData())});$("#plus").click(function(){data[from].total.dg+=
900;TRACK.setTodayKnob();TRACK.storeTrackingData()})});