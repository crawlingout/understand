...
from+"/"+to+"/"+encodeURIComponent(a)),$whentoolong.removeClass("hidden"),$translations.addClass("hidden")),""!==b[from]&&"..."!==b[to]&&$previous_translated_words_wrapper.is(":visible")&&prependPrevWord(b))}function setKnob(a,b){var c=b||stored_audio_time;$knob_player.trigger("configure",{max:a}).val(c).trigger("change")}
...
$("#whentoolong");$play_btn=$("#play_btn");$pause_btn=$("#pause_btn");$play_pause=$("#play_pause");$jumpback=$("#jumpback");$from=$("#from");$to=$("#to");$backhome=$(".backhome");$goal_today=$("#goal_today");$streak=$("#streak");$record_ratio=$("#record_ratio");$previous_translated_words=$("#previous_translated_words");$linktodict=$("#linktodict");$googletranslate=$("#googletranslate");$previous_translated_words_wrapper=$("#previous_translated_words_wrapper");$textFileSelect=$("#textFileSelect");
$audioFileSelect=$("#audioFileSelect");$content=$("#content");$content_wrapper=$("#content_wrapper");$instructions=$("#instructions");$tracking=$("#tracking");controlsToBottom();window.addEventListener("orientationchange",function(){setTimeout(controlsToBottom,400)},!1);var a=$jumpback.data("jump");textfile?loadText(textfile):($instructions.removeClass("hidden"),$backhome.addClass("hidden"));$content.on("click","span.mycontent",function(a){s=window.getSelection();a=s.getRangeAt(0);for(var b=s.anchorNode;0!==
a.toString().indexOf(" ");)a.setStart(b,a.startOffset-1);a.setStart(b,a.startOffset+1);if(a.endOffset<b.length){do a.setEnd(b,a.endOffset+1);while(-1===a.toString().indexOf(" ")&&""!==a.toString().trim())}a=a.toString().trim();handleSelectedText(a)});document.getElementById("content").addEventListener("copy",onCopyEvent);localStorage.getItem("stored_lang_to")||(to=ui_lang,$to.val(to),localStorage.getItem("stored_lang_from")||"en"===to||(from="en",$from.val("en")));$from.val(from);$to.val(to);$from.change(function(){from=
$(this).val();TRACK.dataObjectExist(today);TRACK.setTodayKnob();TRACK.displayTrackingData(today);localStorage.setItem("stored_lang_from",from)});$to.change(function(){to=$(this).val();localStorage.setItem("stored_lang_to",to)});$(".knob").knob({change:function(a){player.currentTime=a}});$knob_player=$("#knob_player");$knob_today=$("#knob_today");$knob_player.trigger("configure",{max:100}).val(0).trigger("change");$play_pause.click(function(){audiofile&&playPause()});$jumpback.click(function(){jumpBack(a)});
var b=document.getElementById("audio_files");b.addEventListener("change",handleAudioFileSelect,!1);document.getElementById("audioFileSelect").addEventListener("click",function(a){b&&b.click();a.preventDefault()},!1);var c=document.getElementById("text_files");c.addEventListener("change",handleTextFileSelect,!1);document.getElementById("textFileSelect").addEventListener("click",function(a){c&&c.click();a.preventDefault()},!1);audiofile&&loadAudioToPlayer(audiofile);$backhome.click(function(){resetPlayer();
resetText()});var d=window.location.search.substring(1);if(d){var e=getQueryVariable(d,"id"),d=getQueryVariable(d,"lang");e&&d&&loadDemo(e,d)}window.onkeyup=function(b){b=b.keyCode?b.keyCode:b.which;37==b?jumpBack(a):39==b&&playPause()};e=$("#r").text();e=e.replace(" at ",String.fromCharCode(64));e=e.replace(" dot ",String.fromCharCode(46));$("#r").text(e);$(".jump_to").click(function(){var a=$(this).data("marker");$body.animate({scrollTop:$("#"+a).offset().top},1E3)});TRACK.dataObjectExist(today);
TRACK.setTodayKnob();TRACK.displayTrackingData(today);data[from].days[today].ga&&($i_am_done.removeClass("hidden"),$idle.html('<i class="fa fa-check-circle"></i>'));TRACK.currentStreak();TRACK.ratioStats();setInterval(function(){player.paused||player.ended?(last_pause_time&&moment().diff(last_pause_time)>allowed_idle&&(document.getElementById("idle").style.color="#d7d7d7"),TRACK.newDay()):TRACK.displayTrackingData(today)},1E3);$idle.click(function(){$body.animate({scrollTop:$tracking[0].scrollHeight+
