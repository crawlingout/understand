...
var TRACK={dataObjectExist:function(a){data[from]||(data[from]={total:{st:0,at:0,dg:1800,s:0,rr:0},days:{}});data[from].days[a]||(data[from].days[a]={at:0,st:0,ga:0})},setTodayKnob:function(){$goal_today.text(convertSeconds(data[from].total.dg));$knob_today.trigger("configure",{max:data[from].total.dg}).val(data[from].days[today].st).trigger("change")},displayTrackingData:function(a){$audio_time.text(convertSeconds(data[from].days[a].at));var b=convertSeconds(data[from].days[a].st);$session_time.text(b);
$session_time_small.text(b);b=calculateRatio(data[from].days[a].at,data[from].days[a].st);0<b?($session_audio_ratio.text(b),$ratio.height(1.7*b).removeClass("hidden")):$ratio.addClass("hidden");$audio_time_total.text(convertSeconds(data[from].total.at));$session_time_total.text(convertSeconds(data[from].total.st));$knob_today.val(data[from].days[a].st).trigger("change");data[from].days[a].ga?data[from].total.rr&&b>data[from].total.rr?$higher_than_ever.removeClass("hidden"):$higher_than_ever.addClass("hidden"):
data[from].days[a].st>data[from].total.dg&&($i_am_done.removeClass("hidden"),$idle.html('<i class="fa fa-check-circle"></i>'),data[from].days[a].ga=1,TRACK.currentStreak())},currentStreak:function(){var a=moment(today).subtract(1,"days").format("YYYY-MM-DD");"added"!==data[from].days[today].ga&&(data[from].days[today].ga?(data[from].total.s=data[from].days[a]&&data[from].days[a].ga?data[from].total.s+data[from].days[today].ga:1,data[from].days[today].ga="added",TRACK.storeTrackingData()):data[from].days[a]&&
data[from].days[a].ga||(data[from].total.s=0));1===data[from].total.s?$streak.text(data[from].total.s+" "+ui_loc[ui_lang].day):$streak.text(data[from].total.s+" "+ui_loc[ui_lang].days)},ratioStats:function(){var a,b;for(a=1;31>a;a++)b=moment(today).subtract(a,"days").format("YYYY-MM-DD")||0,data[from].days[b]&&data[from].days[b].ga&&(data[from].days[b].r||(data[from].days[b].r=calculateRatio(data[from].days[b].at,data[from].days[b].st)),data[from].days[b].r>data[from].total.rr&&(data[from].total.rr=
data[from].days[b].r));data[from].total.rr&&$record_ratio.text(data[from].total.rr+"%")},storeTrackingData:function(){localStorage.setItem("data",JSON.stringify(data))},addToDayAndTotal:function(a,b){data[from].days[today][b]+=a;data[from].total[b]+=a}};deleteTrackingData=function(){localStorage.removeItem("data");document.getElementById("delete").innerHTML="<br /><br />[ deleted ]"};
...
function callBing(a,b,c){$.ajax({type:"POST",data:{authtype:"js"},url:server+"token.php",success:function(d){var e=document.createElement("script");e.src="https://api.microsofttranslator.com/V2/Ajax.svc/Translate?appId=Bearer "+encodeURIComponent(d)+"&from="+encodeURIComponent(a)+"&to="+encodeURIComponent(b)+"&text="+encodeURIComponent(c)+"&oncomplete=mycallback";document.body.appendChild(e)},error:function(a,b){console.log("bing translator error");$translatedword.text("ERROR")}})}
function getTextSelection(){return window.getSelection?window.getSelection().toString():document.getSelection?document.getSelection().toString():document.selection?document.selection.createRange().text:""}function onCopyEvent(){handleSelectedText(getTextSelection())}var previous_selection="";
function handleSelectedText(a){if(!a||previous_selection===a)return!1;previous_selection=a;var b={};b[from]=$selectedword.text();b[to]=$translatedword.text();a=a.trim();1>=a.split(" ").length?($translatedword.text("..."),a=a.replace(/[,.?\u00bf!\u00a1:();\u201e\u201c\u201d\u201a\u2018\u2019"\u2039\u203a\u00ab\u00bb\u2014]/g,""),$whentoolong.addClass("hidden"),$selectedword.text(a),callBing(from,to,a),$translations.removeClass("hidden"),$linktodict.attr("href",linkToDict(a))):($googletranslate.attr("href",
"https://translate.google.com/#"+from+"/"+to+"/"+encodeURIComponent(a)),$translations.addClass("hidden"),$whentoolong.removeClass("hidden"),$translatedword.text("..."));$previous_translated_words.is(":visible")&&"..."!==b[to]&&"ERROR"!==b[to]&&prependPrevWord(b);return!0}function setKnob(a,b){var c=b||stored_audio_time;$knob_player.trigger("configure",{max:a}).val(c).trigger("change")}
...
function resetText(){$backhome.addClass("hidden");$content.addClass("hidden").text("");$instructions.removeClass("hidden");scrollposition=0;localStorage.setItem("scrollposition",scrollposition);localStorage.removeItem("stored_text_file_content");$textFileSelect.css({"background-color":"#4ba3d9",color:"#ffffff"});noParams()}
function handleAudioFileSelect(a){resetPlayer();noParams();audiofile=a.target.files[0];loadAudioToPlayer(URL.createObjectURL(audiofile));a=function(a){return"audio"===a.type.substring(0,5)&&"mp3"===a.name.substr(a.name.length-3)?!0:!1};103809024>audiofile.size&&a(audiofile)&&(a=new FormData($("form")[0]),uploaded_file_id&&a.append("previous",uploaded_file_id),$.ajax({url:server+"upload.php",type:"POST",success:function(a){"Sorry"!==a.substring(0,5)&&(a=$.trim(a),localStorage.setItem("uploaded_file_id",
a),localStorage.setItem("stored_audio_file_url",server+"uploads/"+a+".mp3"))},error:function(a){console.log("error: ",a)},data:a,cache:!1,contentType:!1,processData:!1}))}
function loadText(a){$textFileSelect.css({"background-color":"#FFFFFF",color:"#565656"});a=a.split("\n");for(var b='<p><span class="mycontent"> ',c=0,d=a.length;c<d;c++)b="\r"!==a[c]&&""!==a[c]?b+a[c]+" ":b+'</span></p><p><span class="mycontent"> ';b+="</span></p>";$instructions.addClass("hidden");$backhome.removeClass("hidden");$content.removeClass("hidden").html(b);$content_wrapper.scrollTop(scrollposition);$content_wrapper.scroll(function(){localStorage.setItem("scrollposition",this.scrollTop)})}
...
function loadDemo(a,b){resetPlayer();$.get("../demo/"+a+".txt",function(a){loadText(a)});audiofile="https://www.simplyeasy.cz/understand-server/files/"+a+".mp3";loadAudioToPlayer(audiofile);$from.val(b);$from.change()}function jumpBack(a){var b=player.currentTime;b>a?(player.currentTime=b-a,stored_audio_time=player.currentTime):stored_audio_time=player.currentTime=0;localStorage.setItem("stored_audio_time",stored_audio_time)}
function playPause(){stored_audio_time=player.currentTime;correct_knob_duration&&setTimeout(function(){player.duration&&(setKnob(player.duration,stored_audio_time),correct_knob_duration=0)},200);player.paused||player.ended?(player.play(),$play_btn.addClass("hidden"),$pause_btn.removeClass("hidden"),TRACK.addPauseTime(),document.getElementById("idle").style.color="#4ba3d9"):(player.pause(),$pause_btn.addClass("hidden"),$play_btn.removeClass("hidden"),localStorage.setItem("stored_audio_time",stored_audio_time),
TRACK.startPauseTimer(),TRACK.storeTrackingData())}function controlsToBottom(){var a=0,a=$(window).height(),a=1100>=$(window).width()?a-238:a-168;400<a&&$content_wrapper.css({height:a+"px"})}function getQueryVariable(a,b){for(var c=a.split("&"),d=0;d<c.length;d++){var e=c[d].split("=");if(e[0]==b)return e[1]}return!1}function noParams(){window.location.search.substring(1)&&history.replaceState({},"understand",location.href.split("?")[0])}
$(document).ready(function(){$body=$("html, body");$audio_time=$("#audio_time");$session_time=$("#session_time");$session_time_small=$("#session_time_small");$ratio=$("#ratio");$session_audio_ratio=$("#session_audio_ratio");$audio_time_total=$("#audio_time_total");$session_time_total=$("#session_time_total");$i_am_done=$("#i_am_done");$idle=$("#idle");$higher_than_ever=$("#higher_than_ever");$translatedword=$("#translatedword");$selectedword=$("#selectedword");$translations=$("#translations");$whentoolong=
$("#whentoolong");$play_btn=$("#play_btn");$pause_btn=$("#pause_btn");$play_pause=$("#play_pause");$jumpback=$("#jumpback");$from=$("#from");$to=$("#to");$backhome=$(".backhome");$goal_today=$("#goal_today");$streak=$("#streak");$record_ratio=$("#record_ratio");$previous_translated_words=$("#previous_translated_words");$linktodict=$("#linktodict");$googletranslate=$("#googletranslate");$textFileSelect=$("#textFileSelect");$audioFileSelect=$("#audioFileSelect");$content=$("#content");$content_wrapper=
$("#content_wrapper");$instructions=$("#instructions");$tracking=$("#tracking");controlsToBottom();window.addEventListener("orientationchange",function(){setTimeout(controlsToBottom,400)},!1);var a=$jumpback.data("jump");textfile?loadText(textfile):($instructions.removeClass("hidden"),$backhome.addClass("hidden"));$content.on("click","span.mycontent",function(a){s=window.getSelection();a=s.getRangeAt(0);for(var b=s.anchorNode;0!==a.toString().indexOf(" ");)a.setStart(b,a.startOffset-1);a.setStart(b,
a.startOffset+1);if(a.endOffset<b.length){do a.setEnd(b,a.endOffset+1);while(-1===a.toString().indexOf(" ")&&""!==a.toString().trim())}a=a.toString().trim();handleSelectedText(a)});document.getElementById("content").addEventListener("copy",onCopyEvent);localStorage.getItem("stored_lang_to")||(to=ui_lang,$to.val(to),localStorage.getItem("stored_lang_from")||"en"===to||(from="en",$from.val("en")));$from.val(from);$to.val(to);$from.change(function(){from=$(this).val();TRACK.dataObjectExist(today);TRACK.setTodayKnob();
TRACK.displayTrackingData(today);localStorage.setItem("stored_lang_from",from)});$to.change(function(){to=$(this).val();localStorage.setItem("stored_lang_to",to)});$(".knob").knob({change:function(a){player.currentTime=a}});$knob_player=$("#knob_player");$knob_today=$("#knob_today");$knob_player.trigger("configure",{max:100}).val(0).trigger("change");$play_pause.click(function(){audiofile&&playPause()});$jumpback.click(function(){jumpBack(a)});var b=document.getElementById("audio_files");b.addEventListener("change",
handleAudioFileSelect,!1);document.getElementById("audioFileSelect").addEventListener("click",function(a){b&&b.click();a.preventDefault()},!1);var c=document.getElementById("text_files");c.addEventListener("change",handleTextFileSelect,!1);document.getElementById("textFileSelect").addEventListener("click",function(a){c&&c.click();a.preventDefault()},!1);audiofile&&loadAudioToPlayer(audiofile);$backhome.click(function(){resetPlayer();resetText()});var d=window.location.search.substring(1);if(d){var e=
getQueryVariable(d,"id"),d=getQueryVariable(d,"lang");e&&d&&loadDemo(e,d)}window.onkeyup=function(b){b=b.keyCode?b.keyCode:b.which;37==b?jumpBack(a):39==b&&playPause()};e=$("#r").text();e=e.replace(" at ",String.fromCharCode(64));e=e.replace(" dot ",String.fromCharCode(46));$("#r").text(e);$(".jump_to").click(function(){var a=$(this).data("marker");$body.animate({scrollTop:$("#"+a).offset().top},1E3)});TRACK.dataObjectExist(today);TRACK.setTodayKnob();TRACK.displayTrackingData(today);data[from].days[today].ga&&
($i_am_done.removeClass("hidden"),$idle.html('<i class="fa fa-check-circle"></i>'));TRACK.currentStreak();TRACK.ratioStats();setInterval(function(){player.paused||player.ended?(last_pause_time&&moment().diff(last_pause_time)>allowed_idle&&(document.getElementById("idle").style.color="#d7d7d7"),TRACK.newDay()):TRACK.displayTrackingData(today)},1E3);$idle.click(function(){$body.animate({scrollTop:$tracking[0].scrollHeight+($tracking.offset().top-$(window).height())})});$("#minus").click(function(){900<
data[from].total.dg&&(data[from].total.dg-=900,TRACK.setTodayKnob(),TRACK.storeTrackingData())});$("#plus").click(function(){data[from].total.dg+=900;TRACK.setTodayKnob();TRACK.storeTrackingData()})});